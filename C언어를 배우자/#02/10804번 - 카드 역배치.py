...
for i in range(10):
    ...
    # card에서 시작값, 끝값만큼 slice
    temp = card[start - 1:end]
    # slice한 list를 역순으로 바꾼다.
    temp.reverse()
    # 원래의 card에서 slice한 부분을 제외한다.
    card = list(filter(lambda i: i not in set(temp), card))
    # card에 역순으로 뒤집은 카드목록을 넣어준다.
    for i in range(len(temp)):
        card.insert(start - 1, temp[i])
        start += 1
